...
namespace webs
{
    ...
    /* 删除文件 */
    bool FSUtil::Unlink(const std::string &filename, bool exist = false)
    {
    ...
    bool FSUtil::Rm(const std::string &path)
    {
        ...
        { // 获取文件信息失败（如果lstat函数返回非零值，意味着文件不存在或者无法访问）
            return false;
        }
        // 不是一个目录，使用unlink删除文件
    ...
    bool FSUtil::Mv(const std::string &from, std::string &to)
    { // 文件不存在
